...
void modbus_configure(long baud, unsigned char _slaveID, bool _HalfDuplexEnable, unsigned int _holdingRegsSize, unsigned char _lowLatency)
{
  slaveID = _slaveID;
  Serial1.begin(baud);
  
  if (_HalfDuplexEnable == true) 
  { // To disable set _HalfDuplexEnable to 0
    HalfDuplexEnable = true;
    // DE is connected to A6 of MKR
    pinMode(A6, OUTPUT);	
    digitalWrite(A6, LOW);

    // /RE is connected to A5 of MKR
    pinMode(A5, OUTPUT);	
    digitalWrite(A6, LOW);
...
void sendPacket(unsigned char bufferSize)
{
  if (HalfDuplexEnable == true) {
    digitalWrite(A6, HIGH); // DE 
  ...
  if (HalfDuplexEnable == true) {
    digitalWrite(A6, LOW); // DE 
    digitalWrite(A5, LOW); // /RE
  } 	
}
