  ...
  //showTouch();// method to display touch coordinates on screen
    ...
    switch (tMode) { // which app is currently being monitored for touches
      ...
      case 1:
        ...
        break;
      case 2:
        readKbdTouch(); // watch for Saucy '7' touches
        break;
      default:
        backToMenu();
        break;
...
void readKbdTouch() {
  ...
  char theChar;// to store the current touched character
  if (millis() - lastTouch > tThresh) { // if it's been long enough since last touch
    TSPoint p = ts.getPoint(); // get the screen touch
    ...
    if ((p.z > MINPRESSURE && p.z < MAXPRESSURE)) { // if it's a valid touch
      ...
      if (abs(p.x - 236) < 70 && abs(p.y - 117 < 20)){// menu button
        ...
        delay(100); // slow down sketch a bit
      }
      else {// we have a touch on the screen for a letter

        theChar =  curChar(p.x, p.y); // get the corresponding charcter from keyboard. coords reversed due to landscape
        displayMsg(theChar);// display the message with the current character added to the end
      }
...
void makeKbd() {// draw the keyboard on the screen
  pinMode(YP, OUTPUT);  //restore the TFT control pins
  pinMode(XM, OUTPUT);// for display after detecting a touch
  int cC = 0;// current character to draw on keyboard
  tft.fillScreen(BLACK);
  int xVal = 15;// x,y coordinates for screen objects
  int yVal = 250;
  String curLet = "";// current letter/string that will be drawn on each key
  tft.setTextColor(WHITE); // the text will be white on a blue background
  tft.setTextSize(2);
  for (int i = 1; i <= 6; i++) {
    tft.fillRoundRect(xVal - 10, yVal - 10, 24, 24, 2, BLUE);
    curLet = cString.substring(cC, cC + 1);// grab the current letter to draw on the key
    tft.setCursor(xVal - 3, yVal - 6); // position for the letter

    tft.print(curLet); // print the letter on the key
    cC = cC + 1;// advance to next letter for next key
    xVal = xVal + 34;// advance x-coordinate for next key
  }

}

char curChar(int x, int y) {// a method to determine which character to send back for the message display
  char theChar=32;// empty by default
  for (int i = 0; i < 6; i++) {
    // looking to see if the current x and y coordinate are within bounds of each letter's x and y coordinates
    if ((abs(x - letX[i]) <= 25) && (abs(y - letY[i]) <= 65)) 
      theChar = letter[i]; // if found, set the char to that letter
    }
  
  return theChar;// send the character back to readTouch method
}


void displayMsg(char theChar) {// show the message on the screen
  pinMode(YP, OUTPUT);  //restore the TFT control pins
  pinMode(XM, OUTPUT);// for display after detecting a touch

  msg = msg + theChar; // add current character to the message string
  tft.setCursor(15, 55);// position the cursor
  tft.setTextColor(WHITE);
  tft.setTextSize(2);
  tft.print(msg);// draw the message
}
