...
//	bool isbigend = (flags & 2) != 0;
    ...
    while (n-- > 0) 
	{
		...
		if((MODEL == ILI9488_18)||(MODEL == ILI9486_18))
		{
			writeData18(color);
		}
		else
	...
	else if(lcd_driver == ID_9486)
	{
		...
		switch (rotation) 
		{
		 	...
		 	case 2:
		    	...
		    	break;
		   	case 3:
		     	val = ILI9341_MADCTL_MY | ILI9341_MADCTL_MX | ILI9341_MADCTL_MV | ILI9341_MADCTL_BGR; //270 degree
		     	break;
		 }
		 writeCmdData8(MD, val); 
	}
...
void LCDWIKI_SPI::start(uint16_t ID)
{
	...
	switch(ID)
	{
		...
		case 0x9486:
			...
			if(MODEL == ILI9486_18)
			{
				static const uint8_t ILI9486_IPF[] PROGMEM ={0x3A,1,0x66};
				init_table8(ILI9486_IPF, sizeof(ILI9486_IPF));
			}
			else
			{
				static const uint8_t ILI9486_IPF[] PROGMEM ={0x3A,1,0x55};
				init_table8(ILI9486_IPF, sizeof(ILI9486_IPF));
			}
			//WIDTH = 320,HEIGHT = 480;
			//width = WIDTH, height = HEIGHT;
